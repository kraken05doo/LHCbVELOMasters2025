...
basedir="/home/user309/Documents/mastersProjectCode/selections"
pythondir="/home/user309/Documents/mastersProjectCode/selections/python"
srcdir="/home/user309/Documents/mastersProjectCode/selections/src"
...
from MCTools import * 
gInterpreter.AddIncludePath("/home/user309/Documents/mastersProjectCode/selections/include")
gSystem.Load( "/home/user309/Documents/mastersProjectCode/selections/build/libEvent.so")
...
#events.AddFile((f"/disk/moose/general/djdt/lhcbUII_masters_2025/scoping_phijpsi{particle_type}_1k/u2Selec_tuple_Long.root"))
...
for index,event in enumerate(events):
    ...
    #get photons
    photons = [ track for i,track in enumerate(MC_tracks) if abs(track.ID) == 22 and MC_vertices[i].type == 101 and abs(track.motherID) == 11 ]
    photon_vtxs = [ MC_vertices[i] for i,track in enumerate(MC_tracks) if abs(track.ID) == 22 and MC_vertices[i].type == 101 and abs(track.motherID) == 11 ]
    ...
    for i, Bs_candidate in enumerate(Bs_list):
      ...
      phi_indices = [ phi.mcParticleIndex for phi in phi_list ]
      phi_index = np.where( np.array(phi_indices) == phi.mcParticleIndex )[0][0]
      Jpsi_indices = [ Jpsi.mcParticleIndex for Jpsi in Jpsi_list ]
      Jpsi_index = np.where( np.array(Jpsi_indices) == Jpsi.mcParticleIndex )[0][0]

      k1, k2 = k_used_list[phi_index]
      e1, e2 = e_used_list[Jpsi_index]
      print("Bs candidate: ", Bs.mass * 0.001)

      is_Bs_signal = is_from( e1, MCparticles, 531 ) and is_from( e2, MCparticles, 531 ) and is_from( k1, MCparticles, 531 ) and is_from( k2, MCparticles, 531 )
      print(is_Bs_signal)
      is_Bs_signal *= are_from( e1, e2, MCparticles ) and are_from( k1, k2, MCparticles ) and are_from( e1, k1, MCparticles ) #if e1 & e2, k1 & k2, and e1 & k1 are coupled, no need to check e2 & k2
      print(is_Bs_signal, are_from( phi, Jpsi, MCparticles ))

      #print("e: ", MCparticles[e1.mcParticleIndex].motherID, MCparticles[e1.mcParticleIndex].GDmotherID, MCparticles[e1.mcParticleIndex].GGDmotherID, MCparticles[e1.mcParticleIndex].GGGDmotherID,
      #      "    ", MCparticles[e2.mcParticleIndex].motherID, MCparticles[e2.mcParticleIndex].GDmotherID, MCparticles[e2.mcParticleIndex].GGDmotherID, MCparticles[e2.mcParticleIndex].GGGDmotherID )
      #print("K: ", MCparticles[k1.mcParticleIndex].motherID, MCparticles[k1.mcParticleIndex].GDmotherID, MCparticles[k1.mcParticleIndex].GGDmotherID, MCparticles[k1.mcParticleIndex].GGGDmotherID,
      #      "    ", MCparticles[k2.mcParticleIndex].motherID, MCparticles[k2.mcParticleIndex].GDmotherID, MCparticles[k2.mcParticleIndex].GGDmotherID, MCparticles[k2.mcParticleIndex].GGGDmotherID )
