...
def is_from( track, MCparticles, index ):
  if track.mcParticleIndex == -1 : return False
  if track.mcParticleIndex >= len(MCparticles) : return False 
  mcp = MCparticles[track.mcParticleIndex] 
  return abs(mcp.motherID) == index or abs(mcp.GDmotherID) == index or abs(mcp.GGDmotherID) == index or abs(mcp.GGGDmotherID) == index

def are_from( track1, track2, MCparticles ):
  if track1.mcParticleIndex == -1 or track2.mcParticleIndex == -1 : return False
  if track1.mcParticleIndex >= len(MCparticles) or track2.mcParticleIndex >= len(MCparticles) : return False
  mcp1 = MCparticles[track1.mcParticleIndex]
  mcp2 = MCparticles[track2.mcParticleIndex]
  return mcp1.motherMCParticleIndex == mcp2.motherMCParticleIndex
