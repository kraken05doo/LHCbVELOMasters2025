...
electron_p_change_tuple = ROOT.TNtuple("electron_p_change", "electron_p_change", "initial_P:final_P")
...
for index,event in enumerate(events):
    ...
    displaced_tracks = ROOT.select( particles, vertices, 150, 300, 6 )
    MC_tracks, MC_vertices = MCselect( MCparticles, MCvertices, 100, 100000 )
    ...
    kaons = [ track for track in displaced_tracks if abs(track.trueID) == 321 and track.p4().P() > 1500 and np.sqrt(track.p4().x()**2 + track.p4().y()**2) > 250 ]
    ...
    initial_electron_p = [ electron.p4().P() for electron in electrons_not_reconstructed ]
    ...
    modified_electron_p = [ electron.p4().P() for electron in electrons ]
    for i,initial_p in enumerate(initial_electron_p):
      electron_p_change_tuple.Fill( array.array("f", [initial_p, modified_electron_p[i]]) )
    ...
    print( f"electrons from Jpsi: {np.sum([ 1 if is_from(track, event, 443) else 0 for track in em ])}. positrons from Jpsi: {np.sum([ 1 if is_from(track, event, 443) else 0 for track in ep ])}")
    print( f"Jpsi ID for electrons: {[ MCparticles[track.mcParticleIndex].motherMCParticleIndex for track in em if is_from(track, event, 443) ]}, Jpsi ID for positrons: {[ MCparticles[track.mcParticleIndex].motherMCParticleIndex for track in ep if is_from(track, event, 443) ]} \n\n")
    ...
    for i,phi_candidate in enumerate(phi_candidates):
        ...
        is_phi_signal *= are_from(k1, k2, event) #are the K's from the same phi using MCParticleIndex

        #print(is_from(k1, event, 333) and is_from(k2, event, 333), are_from(k1, k2, event))
    ...
    for j,Jpsi_candidate in enumerate(Jpsi_candidates):
        ...
        is_Jpsi_signal *= are_from(e1, e2, event) #are the J/psi's from the same phi using MCParticleIndex

        #print(is_from(e1, event, 443) and is_from(e2, event, 443), are_from(e1, e2, event))
    ...
    Bs_list = ROOT.combine(phi_list, Jpsi_list, doca_cut, 15, 0)

    for i, Bs_candidate in Bs_list:
      phi, Jpsi, Bs, Bs_vtx = Bs_candidate

      is_Bs_signal = is_from( phi, event, 531 ) and is_from( Jpsi, event, 531 )
      is_Bs_signal *= are_from( phi, Jpsi, event )
