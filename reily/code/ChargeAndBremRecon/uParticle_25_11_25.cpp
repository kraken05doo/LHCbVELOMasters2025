...
std::vector<uParticle> bremRecon( std::vector<uParticle> input,
                                  const std::vector<uMCParticle> MCparticles,
                                  const std::vector<uMCVertex> MCvertices,
                                  const float minP,
                                  const float minE,
                                  const float minZ,
                                  const float maxZ )
{
  ...
  for( int i=0; i<MCparticles.size(); i++ ) {
    if( MCparticles[i].ID != 22 and MCvertices[i].type != 101 and MCparticles[i].motherID != 11 ) continue;
    if( MCparticles[i].p4().E() < minE ) continue;
    if( sqrt( MCparticles[i].p4().x() * MCparticles[i].p4().x() + MCparticles[i].p4().y() * MCparticles[i].p4().y() + MCparticles[i].p4().z() * MCparticles[i].p4().z() ) < minP ) continue;
    if( MCvertices[i].pos.z() < minZ ) continue;
    if( MCvertices[i].pos.z() > maxZ ) continue;

    bremPhotons.push_back(MCparticles[i]);
    bremPhotonVertices.push_back(MCvertices[i]);
    bremPhotonMotherMCID.push_back(MCparticles[i].motherMCParticleIndex);
  }
