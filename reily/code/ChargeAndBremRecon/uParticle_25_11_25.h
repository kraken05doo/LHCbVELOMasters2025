...
struct uParticle  {
  ...
  int charge() const { 
    //return firstState.qop > 0 ? +1 : -1 ;
    if (firstState.charge > 950000) { //some leniency on the 1 million value in case large change in charge
      return firstState.qop > 0 ? +1 : -1 ; 
    } else {
      return firstState.charge ;
    }
  }
...
std::vector<uParticle> bremRecon( std::vector<uParticle> input,
                                  ...
                                  const float minP = 100, 
                                  const float minE = 100,
                                  const float minZ = 0,
                                  const float maxZ = 6000  ); //z=6000 ~where magnet is in LHCb 
